...
import type { ClassroomLink } from "@/app/student/data/classroomData";
...
export default function ContractSchoolsPage() {
  ...
  const removeClassroom = (classroomId: string) => {
    if (!selectedSchool) return;
    const nextVisibility = { ...selectedSchool.lessonVisibility };
    delete nextVisibility[classroomId];
    setSelectedSchool({
      ...selectedSchool,
      classrooms: selectedSchool.classrooms.filter(
        (classroom) => classroom.id !== classroomId
      ),
      lessonVisibility: nextVisibility,
    });
  };
            ...
            <section className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
              {orderedSchools.map((school) => (
                <button key={school.slug} type="button" onClick={() => selectSchool(school)} className="rounded-[24px] border border-sky-100 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-sky-300 hover:shadow-lg sm:p-5">
                  ...
                  <div className="mt-1 text-xs font-bold text-slate-400">/{school.slug} · {school.classrooms.length}개 반 · {school.lessons.length}차시</div>
            ...
            <div className="rounded-[28px] bg-white p-5 shadow-lg sm:p-7">
              <div className="flex flex-wrap items-end justify-between gap-3">
                ...
                <div className="flex items-end gap-2">
                  ...
                  <button type="button" onClick={addClassroom} className="rounded-xl bg-sky-500 px-3 py-2 text-sm font-black text-white">+ 반 추가</button>
                </div>
              </div>
              <div className="mt-4 flex flex-wrap gap-2">
                {selectedSchool.classrooms.map((classroom) => <div key={classroom.id} className="flex items-center gap-2 rounded-2xl border border-sky-100 bg-sky-50 px-3 py-2 text-sm font-black text-sky-800"><span>{classLabel(classroom)}</span><button type="button" aria-label={`${classLabel(classroom)} 삭제`} onClick={() => removeClassroom(classroom.id)} className="text-rose-500">×</button></div>)}
                {selectedSchool.classrooms.length === 0 && <div className="text-sm font-bold text-slate-400">등록된 반이 없어요.</div>}
              </div>
              <button type="button" disabled={saving} onClick={() => void saveSchool(selectedSchool, "학년·반 구성을 저장했어요.")} className="mt-4 rounded-2xl bg-sky-100 px-4 py-2.5 text-sm font-black text-sky-700 disabled:opacity-50">학년·반 저장</button>
            </div>
                ...
                {selectedSchool.lessons.map((lesson) => {
                  ...
                  return <article key={lesson.id} className="overflow-hidden rounded-[22px] border border-slate-200">
                    ...
                    {expanded && <div className="space-y-4 border-t border-slate-100 p-4">
                      ...
                      <div>
                        <div className="flex items-center justify-between"><div className="text-sm font-black text-slate-800">링크 목록</div><button type="button" onClick={() => updateLessonLinks(lesson.id, (links) => [...links, { id: makeClientId("link"), label: "새 링크", href: "https://", kind: "activity", defaultUnlocked: true }])} className="rounded-xl bg-sky-100 px-3 py-2 text-xs font-black text-sky-700">+ 링크 추가</button></div>
