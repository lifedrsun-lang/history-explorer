...
export default function TeacherFeePanel() {
  ...
  const allFeeRows = useMemo(() => {
    ...
    for (const student of students) {
      if (getEnrollmentStatus(student) !== "active") continue;
      if (getStudentProgramValue(student?.program) === "boardgame") continue;

      const teachingClass = getTeachingClass(student);
      if (!teachingClass) continue;

      const contract = getTeacherFeeContract(student?.school, teachingClass);
  ...
  const schoolOptions = useMemo(
    () => [
      "전체학교",
      ...Array.from(new Set(allFeeRows.map((row) => row.schoolName))),
    ],
    [allFeeRows]
  );
