...
type Student = { id:string; name:string; school:string; grade:string; schoolClass:string; enrollmentStatus:string };
type School = { contractId:string; schoolName:string; title:string; rule:string; quarterParticipation:Partial<Record<Quarter,Record<string,boolean[]>>>; students:Student[] };
type SavedRecord = { contractId?:string; quarter?:string; receipts?:Record<string,boolean[]> };
const quarters:{key:Quarter;label:string}[]=[{key:"Q1",label:"1분기"},{key:"Q2",label:"2분기"},{key:"Q3",label:"3분기"},{key:"Q4",label:"4분기"}];
...
export default function TextbookReceiptsPage(){
  ...
  useEffect(()=>onAuthStateChanged(auth,(current)=>{setUser(current);setAuthChecking(false)}),[]);

  const requestJson=useCallback(async(url:string,init?:RequestInit)=>{
    if(!user) throw new Error("교사 로그인이 필요합니다.");
    const token=await user.getIdToken();
    const response=await fetch(url,{...init,cache:"no-store",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`,...(init?.headers||{})}});
  ...
  if(authChecking)return <main className="min-h-screen bg-slate-50 p-6 font-bold">로그인 확인 중...</main>;
  if(!user)return <main className="min-h-screen bg-slate-50 p-6 font-bold">교사 로그인이 필요합니다.</main>;
      ...
      <section className="mt-4 overflow-hidden rounded-[28px] bg-white shadow-sm">
        <div className="overflow-x-auto">
          <div className="min-w-[650px]">
            <div className="grid grid-cols-[120px_100px_1fr_80px_80px_80px] bg-slate-900 px-4 py-3 text-center text-xs font-black text-white">
              <div className="text-left">학년·반</div><div>상태</div><div className="text-left">이름</div>{terms.map(t=><div key={t}>{t}</div>)}
            </div>
            {loading?<div className="p-8 text-center text-sm font-bold text-slate-400">불러오는 중...</div>:visibleStudents.length===0?<div className="p-8 text-center text-sm font-bold text-slate-400">이 분기에 수강 체크된 학생이 없습니다.</div>:visibleStudents.map((student)=><div key={student.id} className="grid grid-cols-[120px_100px_1fr_80px_80px_80px] items-center border-t border-slate-100 px-4 py-3 text-center">
              <div className="text-left text-sm font-black text-slate-700">{[student.grade,student.schoolClass].filter(Boolean).join(" ")||"-"}</div>
              <div className="text-xs font-bold text-slate-400">{student.enrollmentStatus==="ended"?"종료":student.enrollmentStatus==="paused"?"쉬는중":"수강중"}</div>
              <div className="text-left text-sm font-black text-slate-900">{student.name}</div>
              {[0,1,2].map((term)=><label key={term} className="flex justify-center"><input type="checkbox" checked={Boolean(receipts[student.id]?.[term])} onChange={()=>toggle(student.id,term)} className="h-5 w-5 accent-indigo-600"/></label>)}
            </div>)}
          </div>
        </div>
        <div className="border-t border-slate-200 bg-slate-50 p-5">
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <div className="rounded-2xl bg-white p-3 text-center"><div className="text-[11px] font-black text-slate-400">수령 학생</div><div className="mt-1 text-xl font-black">{uniqueCount}명</div></div>
            {totals.map((count,index)=><div key={index} className="rounded-2xl bg-white p-3 text-center"><div className="text-[11px] font-black text-slate-400">{terms[index]}</div><div className="mt-1 text-xl font-black">{count}명</div></div>)}
          </div>
          <button type="button" onClick={()=>void save()} disabled={!school||saving} className="mt-4 w-full rounded-2xl bg-indigo-600 py-3 text-sm font-black text-white disabled:opacity-50">{saving?"저장 중...":"이 분기 수령인원 확정 저장"}</button>
        </div>
      </section>
    </div>
  </main>;
