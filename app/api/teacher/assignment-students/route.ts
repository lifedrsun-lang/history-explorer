...
const getTargetStudentKeys = (data: FirebaseFirestore.DocumentData) =>
...
export async function GET(request: Request) {
  try {
    ...
    const [snapshots, assignmentSnapshot, submissionSnapshot, reviewAssignmentSnapshot, reviewCompletionSnapshot] =
      await Promise.all([
        Promise.all(
    ...
    assignmentSnapshot.docs.forEach((docItem) => {
      ...
      const targets = new Set(getTargetStudentKeys(data));
