...
const loadPdfJs = () => {
  ...
  pdfJsLoader = new Promise<void>((resolve, reject) => {
    const existing = document.querySelector<HTMLScriptElement>(
      'script[data-sunlab-pdfjs="true"]'
    );

    const ready = () => {
      if (!window.pdfjsLib) {
        reject(new Error("PDF 뷰어를 준비하지 못했습니다."));
        return;
      }
      window.pdfjsLib.GlobalWorkerOptions.workerSrc = PDFJS_WORKER;
      resolve();
    ...
    script.async = true;
    script.defer = true;
    script.dataset.sunlabPdfjs = "true";
    script.onload = ready;
    script.onerror = () => reject(new Error("PDF 뷰어를 불러오지 못했습니다."));
...
export default function PdfBookReader({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        await loadPdfJs();
        if (!window.pdfjsLib) throw new Error("PDF 뷰어를 준비하지 못했습니다.");
        ...
        setPdf(loadedPdf);
        setPage(clamp(Number.isFinite(savedPage) ? savedPage : 1, 1, loadedPdf.numPages));
      } catch (loadError) {
        console.error("Failed to open Sun Lab PDF", loadError);
        if (active) {
          setError("책을 열지 못했습니다. 선생님께 Drive 연결 상태를 확인해 달라고 해주세요.");
        }
      } finally {
  ...
  const goPrevious = () => setPage((current) => Math.max(1, current - step));
  const goNext = () => setPage((current) => Math.min(maxStartPage, current + step));
  const zoomOut = () => setScale((current) => clamp(Number((current - 0.15).toFixed(2)), 0.7, 1.9));
  const zoomIn = () => setScale((current) => clamp(Number((current + 0.15).toFixed(2)), 0.7, 1.9));
  ...
  return (
    ...
    >
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              onClick={zoomOut}
              disabled={scale <= 0.7}
              className="rounded-xl bg-white/10 px-3 py-2 text-sm font-black disabled:opacity-30"
              aria-label="축소"
            >
              −
            </button>
            <div className="min-w-14 text-center text-xs font-black text-slate-300">
            ...
            <button
              ...
              className="hidden rounded-xl bg-white/10 px-3 py-2 text-xs font-black md:block"
            >
              {spread ? "한쪽 보기" : "두쪽 보기"}
            </button>
          </div>
      ...
      <main className="mx-auto max-w-[1600px] px-2 py-4 sm:px-4">
        {loading ? (
          <div className="flex min-h-[65vh] items-center justify-center text-center text-sm font-bold text-slate-300">
            📖 책을 여는 중이에요...
          </div>
        ) : error && !pdf ? (
          <div className="mx-auto mt-16 max-w-md rounded-3xl border border-rose-400/30 bg-rose-950/40 p-6 text-center">
            ...
            <Link
              href="/student/library"
              className="mt-5 inline-flex rounded-xl bg-white px-4 py-2 text-xs font-black text-slate-900"
            >
              책장으로 돌아가기
            </Link>
          </div>
          ...
          <>
                ...
                <div className="relative overflow-hidden rounded-lg bg-white shadow-2xl">
                  ...
                  <div className="pointer-events-none absolute inset-0 grid grid-cols-2 grid-rows-3 overflow-hidden opacity-[0.075]">
                    {Array.from({ length: 6 }).map((_, index) => (
                      <div
                        key={index}
                        className="flex -rotate-12 items-center justify-center whitespace-nowrap px-4 text-[10px] font-black text-slate-900 sm:text-xs"
                      >
                        {watermarkText}
                      </div>
                    ))}
            ...
            {error && (
              <div className="mx-auto mt-3 max-w-xl rounded-xl bg-rose-950/50 px-4 py-2 text-center text-xs font-black text-rose-200">
                {error}
              </div>
            )}
